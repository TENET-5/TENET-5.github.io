...
if (-not $PythonExe) { 
    ...
    if ($PythonExe) {
        ...
        $PythonExe = "$env:LOCALAPPDATA\Microsoft\WindowsApps\pythonw.exe" 
    }
}
...
$action = New-ScheduledTaskAction `
    -Execute $PythonExe `
    -Argument "`"$Orchestrator`" --force --deploy" `
...
# Phase 71 Infrastructural Synchronization Target Limits
Write-Host "`n[PHASE 71] Sync Target: track_scheduler_configurations -> endpoint_scheduler_configurations [LOCKED]" -ForegroundColor Cyan
Write-Host "[PHASE 71] Sync Target: monitor_scheduler_execution -> endpoint_scheduler_execution [LOCKED]" -ForegroundColor Cyan
