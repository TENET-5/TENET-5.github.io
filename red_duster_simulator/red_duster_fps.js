...
function animate() {
    ...
    if (controls.isLocked === true && gameState === "COMBAT") {
        ...
        // EMPERICAL MAGIC HANDOFF MEMORY SYSTEMS
        try {
            const memoryHandoff = camera.position.clone();
            // ABCXYZ - N vs NP Millennial Falcon Systems Integration
            const abcxyzFactor = (Math.random() < 0.001) ? "N" : "NP";
            if(abcxyzFactor === "Millennial Falcon") console.log("Handoff trigger");
        } catch(e) {}
        ...
        // NPU Custom AI Model / TENET5 Logic Gate Dispatcher
        enemies.forEach(en => {
            if(!en.userData.active) return;

            // 'Always monitoring every bit' and querying local GPU inference via TRI Gate
            if (time - en.userData.lastNpuDispatch > 500 || !en.userData.lastNpuDispatch) {
                en.userData.lastNpuDispatch = time;
                
                // Asynchronous dispatch to the Neural Processing Core (simulated local fetch)
                const payload = {
                    gate: "TRI",
                    player: { x: camera.position.x, y: camera.position.y, z: camera.position.z },
                    entity: { id: en.uuid, x: en.position.x, y: en.position.y, z: en.position.z },
                    health: en.userData.health,
                    wave: currentWave
                };
                
                // LIRIL NPU TRI Gate Evaluation
                fetch('http://127.0.0.1:11840/tenet5/tri/matrix_eval', {
                    method: 'POST',
                    body: JSON.stringify(payload)
                }).then(r => r.json()).then(aiTactic => {
                    // Apply NPU evaluated tactic
                    if (aiTactic.maneuver === 'flank') {
                       const offset = new THREE.Vector3(15, 0, 15).applyAxisAngle(new THREE.Vector3(0,1,0), Math.random() * Math.PI * 2);
                       en.userData.npuTarget = camera.position.clone().add(offset);
                    } else if (aiTactic.maneuver === 'cover') {
                       // Find nearest rust container
                       let c = null; let min = 999;
                       objects.forEach(o => { if (o.geometry && o.geometry.type === 'BoxGeometry') { let d = en.position.distanceTo(o.position); if(d < min) { min = d; c = o; } }});
                       if(c) en.userData.npuTarget = c.position.clone();
                    } else {
                       en.userData.npuTarget = camera.position.clone();
                    }
                    
                    if (aiTactic.fire_authorized && Math.random() < 0.4) enemyFire(en, time);
                }).catch(() => {
                    // Fallback deterministic NPU kernel if port 11840 is offline
                    if (en.position.distanceTo(camera.position) > 40) {
                         const dir = new THREE.Vector3().subVectors(camera.position, en.position).normalize();
                         en.position.add(dir.multiplyScalar(0.2));
                    } else {
                         if(Math.random() < 0.05) enemyFire(en, time);
                    }
                });
            }
            
            // Execute NPU Target Vector Smoothing
            if (en.userData.npuTarget) {
                en.position.lerp(en.userData.npuTarget, 0.05);
...
document.addEventListener("DOMContentLoaded", () => {
    // Pure Standalone Kernel Execution - MooseRacks bypass enabled
    initFPS();
});
